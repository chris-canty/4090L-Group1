...
func cardSingleTarget( 	rawDamage: int ,accuracy : int, 
						...
						color: String, vector:Vector2=Vector2(0,0)):
	...
	if roll <= accuracy:
		...
		text_instance.get_node("Text").set("theme_override_colors/font_shadow_color",Color(color))
...
=======
	...
	match action_id:
		...
		1:
			...
			color = "d5621f"
			cardSingleTarget(raw ,accuracy,mp_cost, nameSpell, status, addrScen, color)
			#for c in combatants:
				#await c.disable_bars()
		...
		2:
			...
			color = "c3d511"			
			vector = Vector2(0,58)
			cardSingleTarget(raw ,accuracy, mp_cost,nameSpell, status, addrScen,color, vector)
			#for c in combatants:
				#await c.disable_bars()
